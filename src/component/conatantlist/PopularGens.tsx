...
import { list, populardata } from '../../data/data';
import PopulargenList from './PopulargenList';
const PopularGens = () => {
  ...
  const handleClick = (direction: string) => {
    ...
    if (direction === "right" && sliderNumber < 4) {
      setSliderNumber(sliderNumber + 1);
      listRef.current.style.transform = `translateX(${-1050 + distance }px)`;
    } 
  };
  return (
     ...
     <div className="list__container slider" ref={listRef}>
       
       {
           populardata.map((item,index)=>{
               return <PopulargenList moviedata={item} index={index} key={index}/>
           })
